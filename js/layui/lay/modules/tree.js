...
layui.define('form', function (exports) {
  ...
  Class.prototype.tree = function (elem, children) {
    ...
    layui.each(data, function (index, item) {
      var hasChild = item.children && item.children.length > 0,
        ...
        entryDiv = $(['<div data-id="' + item.id + '" class="layui-tree-set' + (item.spread ? " layui-tree-spread" : "") + (item.checked ? " layui-tree-checkedFirst" : "") + '">', '<div ' + (options.drag && !item.fixed ? 'draggable="true"' : '') + ' class="layui-tree-entry">', '<div class="layui-tree-main">'
          //箭头
          ,
          function () {
            if (options.showLine) {
  ...
  Class.prototype.spread = function (elem, item) {
    ...
    touchOpen.on('click', function (e) {
      ...
      options.click && options.click({
        ...
        state: 'iconClick',
        data: item,
        isExpand: elem.hasClass(ELEM_SPREAD)
      });
    });
  ...
  var addToItem = function (treeData, parentItem, subItems) {
    var isAdded = false;

    for (var i = 0; i < treeData.length; i++) {
      if (treeData[i].id == parentItem.id) {
        if (treeData[i].children == undefined) {
          treeData[i].children = [];
        }
        for (var j = 0; j < subItems.length; j++) {
          treeData[i].children.push(subItems[j]);
        }
        isAdded = true;
        return false;
      } else if (treeData[i].children != undefined && treeData[i].children.length > 0) {
        isAdded = addToItem(treeData[i].children, parentItem, subItems);
  ...
  tree.addSubItem = function (id, data, subItems) {
    var that = thisModule.that[id];
    addToItem(that.config.data, data, subItems);
    that.reload({});
    return thisModule.call(that);
  }

  var addAllSubItems = function (treeData, parentItem, subItems) {
    var isAdded = false;
    ...
    for (var i = 0; i < treeData.length; i++) {
      if (treeData[i].id == parentItem.id) {
        ...
        isAdded = addAllSubItems(treeData[i].children, parentItem, subItems);
        if (isAdded) return false;
      }
    }
  ...
  tree.addAllSubItems = function (id, data, subItems) {
    var that = thisModule.that[id];
    addAllSubItems(that.config.data, data, subItems);
    that.reload({});
    return thisModule.call(that);
  }

  var removeSubItems = function (treeData, parentItem) {
    var isRemoved = false;
    ...
    for (var i = 0; i < treeData.length; i++) {
      if (treeData[i].id == parentItem.id) {
        ...
        isRemoved = removeSubItems(treeData[i].children, parentItem);
  ...
  tree.removeSubItems = function (id, data) {
    var that = thisModule.that[id];
    removeSubItems(that.config.data, data);
    that.reload({});
    return thisModule.call(that);
  }
